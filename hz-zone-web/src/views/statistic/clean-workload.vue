...
import { listWorkload } from '@/api/statistic/cleanWorkload'
...
export default {
  ...
  data() {
    return {
      ...
      role: {
        list: '/base/cleanReport/workload',
        download: {
          url: '/base/cleanReport/workloadDownload',
          title: '清点员工作量月度统计'
        }
      },
      tableList: [
  ...
  methods: {
    ...
    async getJobNameOption() {
      ...
      await jobNameOption(this.listQuery.departmentId, '5').then(res => {
        this.options.empId = res.data[5]
      })
    },
    formatMoney,
